...
export default function RAGSystemPrototype() {
  ...
  }, [currentScreen, selectedDocument, messages.length]);

  const navigateTo = (screen: string, doc: any = null) => {
  ...
  return (
    <div style={styles.app}>
      {/* Prototype Indicator */}
      <div style={styles.prototypeIndicator}>📱 Interactive React Prototype</div>

      {/* Landing Screen */}
      {currentScreen === 'landing' && <LandingScreen onGetStarted={() => navigateTo('upload')} />}

      {/* Upload Screen */}
      {currentScreen === 'upload' && <UploadScreen onNavigate={navigateTo} />}
      ...
      {currentScreen === 'library' && (
        <LibraryScreen
          ...
          onSelectDocument={(doc: any) => navigateTo('chat', doc)}
        />
      )}
  ...
  return (
    <div style={styles.landing}>
      ...
      <div style={styles.landingContent}>
        ...
        <p style={styles.landingSubtitle}>AI-Powered Document Intelligence for Learning & Research</p>
        <button style={styles.btnPrimary} onClick={onGetStarted}>
          Get Started →
        </button>

        <div style={styles.featureGrid}>
          <FeatureCard icon="📚" title="Smart Upload" description="Support for PDFs and various learning materials" />
          <FeatureCard
            icon="🔍"
            title="Semantic Search"
            description="Vector embeddings for intelligent retrieval"
          />
          <FeatureCard icon="💬" title="Conversational AI" description="Natural language Q&A with source citations" />
        </div>
      </div>
    </div>
  );
...
function UploadScreen({ onNavigate }: { onNavigate: (screen: string) => void }) {
  return (
    <div style={styles.screen}>
      ...
      <div style={styles.mainContent}>
        ...
        <p style={styles.securityNote}>Your documents are processed securely with end-to-end encryption</p>
      </div>
    </div>
  );
}
...
function DocumentCard({ doc, onClick }: { doc: any; onClick: () => void }) {
  ...
  return (
    ...
    >
      <div style={{ ...styles.docIcon, background: doc.color }}>{doc.icon}</div>
...
function ChatScreen({
  ...
  // FIX: useRef instead of useState for DOM refs
        ...
        <div style={styles.chatMain}>
          ...
          <div style={styles.inputArea}>
            <input
              ...
              onKeyDown={(e) => e.key === 'Enter' && onSend(inputValue)} // FIX: onKeyPress -> onKeyDown
            />
            <button style={styles.sendBtn} onClick={() => onSend(inputValue)}>
              Send
            </button>
          </div>
        </div>
...
function Sidebar({ document, onSend }: { document: any; onSend: (text: string) => void }) {
  return (
    <div style={styles.sidebar}>
      ...
      <div style={{ marginTop: 30 }}>
        ...
        <div style={{ marginTop: 15, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <button style={styles.quickAction} onClick={() => onSend('Summarize this document')}>
            📋 Summarize document
          </button>
          <button style={styles.quickAction} onClick={() => onSend('Extract key concepts')}>
            🔑 Extract key concepts
          </button>
          <button style={styles.quickAction} onClick={() => onSend('Generate quiz questions')}>
        ...
        {message.suggestions && (
          <div style={styles.suggestions}>
            {message.suggestions.map((suggestion: string, i: number) => (
              <button key={i} style={styles.suggestionChip} onClick={() => onSuggestionClick(suggestion)}>
                {suggestion}
              </button>
            ))}
          </div>
          ...
          <div style={styles.citations}>
            ...
            <div style={styles.citationItems}>
              {message.citations.map((citation: any, i: number) => (
                <span key={i} style={styles.citationBadge} onClick={() => onCitationClick(citation)}>
                  {citation.text}
                </span>
              ))}
            </div>
...
// Header Component
function Header({ onNavigate, activeTab }: { onNavigate: (screen: string) => void; activeTab: string }) {
  return (
    <div style={styles.header}>
      <div style={styles.logo}>📖 RAG Notebook</div>
      <div style={styles.navTabs}>
        <div style={activeTab === 'upload' ? styles.navTabActive : styles.navTab} onClick={() => onNavigate('upload')}>
          Upload
        </div>
        <div
